...
while True:
    ...
    if flag and not ((frame_count - frames[-1]) % factor):
        ...
        if  (frame_count - frames[-1]) in [0, fps, 2*fps]:
            ...
            cv2.imwrite(os.path.sep.join(["snapshots", dir, "frame_{}.jpg".format(frame_count)]), frame)

		# check if the video writer is None
        if writer is None:
			# initialize our video writer
            fourcc = cv2.VideoWriter_fourcc(*'XVID')
            writer = cv2.VideoWriter(args["output"], fourcc, fps//2, (frame.shape[1], frame.shape[0]), True)
...
for i in range(0, len(time_stamp)-1, 2):
    time_of_movements = time_of_movements.append({"Start":time_stamp[i], "End":time_stamp[i + 1]}, ignore_index = True)
    frames_of_movements = frames_of_movements.append({"Start":frames[i], "End":frames[i + 1]}, ignore_index = True)
